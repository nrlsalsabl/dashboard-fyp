<button type="button" data-modal-target="edit-data-modal-{{ $performance->id }}"
    data-modal-toggle="edit-data-modal-{{ $performance->id }}"
...
<div class="fixed left-0 right-0 z-50 items-center justify-center hidden overflow-x-hidden overflow-y-auto top-4 md:inset-0 h-modal sm:h-full"
    id="edit-data-modal-{{ $performance->id }}">
                ...
                <form action="/kinerja-intern/{{ $performance->id }}" method="POST">
                    ...
                    <div class="grid grid-cols-6 gap-6">
                        <div class="col-span-6">
                            <label for="intern_id"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Intern</label>
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            <label for="result"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Capaian</label>
                            <input type="number" step="0.01" name="result" id="result"
                                value="{{ old('result', $performance->result) }}"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="3,25" required>
                        </div>
                        <div class="col-span-6 sm:col-span-3">
                            <label for="date"
                            ...
                            <div class="relative">
                                ...
                                <input datepicker datepicker-autohide datepicker-format="yyyy-mm-dd" type="text"
                                    ...
                                    placeholder="Pilih tanggal" value="{{ $performance->date }}" required>
                            </div>
                        </div>
                        <div class="col-span-6">
                            <label for="description"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Justifikasi</label>
                            <textarea id="description" name="description" rows="4"
                                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Membuat dashboard.." required>{{ $performance->description }}</textarea>
                        </div>
                    </div>
            </div>
            <!-- Modal footer -->
