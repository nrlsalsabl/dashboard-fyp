                ...
                <div class="overflow-hidden shadow">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @foreach ($tables as $scope)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700"
                                    id="{{ $search }}_ids{{ $scope->id }}">
                                    <td class="w-4 p-4">
                                        <div class="flex items-center">
                                    ...
                                    <td
                                        class="w-full p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $scope->name }}</td>
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        <!-- Edit User Modal -->
                                        @include('scope.edit')
                                        @can('delete data')
                                            <form action="/scope/{{ $scope->id }}" method="POST" class="inline-flex">
                                                @method('delete')
                                                @csrf
                                                <!-- Delete User Modal -->
                                                @include('scope.delete')
                                            </form>
                                        @endcan
                                    </td>
                                </tr>
