<button type="button" data-modal-target="edit-data-modal-{{ $scope->id }}"
    data-modal-toggle="edit-data-modal-{{ $scope->id }}"
    ...
    <div class="relative w-full h-full max-w-2xl px-4 md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <div class="p-6 space-y-6">
                <form action="/scope/{{ $scope->id }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="grid grid-cols-6 gap-6">
                        <div class="col-span-6">
                            <label for="name"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Data Scope</label>
                            <input type="text" name="name" value="{{ old('name', $scope->name) }}" id="name"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Masukan Scope" required>
                        </div>
                    </div>
            </div>
            <!-- Modal footer -->
